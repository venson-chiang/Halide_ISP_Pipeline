...
public:
    ...
    void generate() {
        ...
        // Now interpolate diagoally to get red at blue and blue at
        // red. Hold onto yout hats; this gets really fancy. We do the
        ...
        // sites - we correct out interpolations using the second 
        ...
        // seperately. Everything else we'll just inline.
    ...
    void schedule() {
        ...
        intermediates[1].compute_with(intermediates[0], x,
            {{x, LoopAlignStrategy::AlignStart}, {y, LoopAlignStrategy::AlignEnd}});
...
private:
    // Intermediate stencil stages to scheduel
...
Func CameraPipe::deinterleave(Func raw) {
    ...
    deinterleaved(x, y ,c) = mux(c, {raw(2*x, 2*y), raw(2*x+1, 2*y),
...
Func CameraPipe::apply_curve(Func input) {
    ...
    // How much to upsameple the LUT by when sampling it.
    int lutResample = 1;
    minRaw /= lutResample;
    maxRaw /= lutResample;
    ...
    Expr z = select(g > 0.5f, 1.0f - (a * (1.0f-g) * (1.0f-g) + b * (1.0f-g)),
                              a * g * g + b * g);
...
Func CameraPipe::sharpen(Func input) {
    // Convert the sharpening strength to 2.5 fixed point. This allows sharpening int the 
    // range [0, 4].
    ...
    Func mask("mask");
    mask(x, y, c) = cast<int16_t>(input(x, y, c)) - cast<uint16_t>(unsharp(x, y, c));
...
void CameraPipe::generate() {
    ...
    // boundaries so that we don't need to check bouns. We're going
    ...
    /* Estimates */
    // (This can be useful in conjunction with RenGen and benchmarks as well
     ...
     /* Optional tags to specify layout for HalideTraceViz */
    {
        ...
        demosaiced->output.add_trace_tag(cfg.to_trace_tag());
        ...
        cfg.max = 256;
        cfg.pos = {1660, 360};
        cfg.labels = {{"sharpened(final image)"}};
